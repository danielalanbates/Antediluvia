...
/// Tags cloud entities for drift animation.
#[derive(Component)]
    ...
    App::new()
        ...
        .add_systems(
            ...
            (
                ...
                foliage::wind_animation_system,
                particles::campfire_emitter_system,
                particles::particle_update_system,
                particles::dust_mote_system,
                cloud_system,
            )
            .run_if(in_state(AppState::InWorld)),
        )
...
/// Drift clouds across the sky and adjust opacity with day/night.
fn cloud_system(
    time: Res<Time>,
    cycle: Res<DayNightCycle>,
    mut cloud_q: Query<(&CloudEntity, &mut Transform, &MeshMaterial3d<StandardMaterial>)>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    let t = time.elapsed_secs();
    let hour = cycle.time_of_day;

    // Cloud opacity: visible during day, fading at night
    let base_alpha = if hour >= 6.0 && hour < 18.0 {
        0.35
    } else if hour >= 18.0 && hour < 20.0 {
        0.35 - (hour - 18.0) / 2.0 * 0.25 // fade out at dusk
    } else if hour >= 4.0 && hour < 6.0 {
        0.10 + (hour - 4.0) / 2.0 * 0.25 // fade in at dawn
    } else {
        0.10 // dim at night
    };

    for (cloud, mut transform, mat_handle) in cloud_q.iter_mut() {
        // Drift position with wrapping
        let dx = (t * cloud.drift_speed.x * 0.1).sin() * cloud.drift_range * 0.5;
        let dz = (t * cloud.drift_speed.y * 0.07 + 1.0).cos() * cloud.drift_range * 0.3;

        transform.translation.x = cloud.origin.x + dx;
        transform.translation.z = cloud.origin.y + dz;

        // Subtle vertical bob
        transform.translation.y += (t * 0.15 + cloud.origin.x * 0.01).sin() * 0.02;

        // Update opacity
        if let Some(mat) = materials.get_mut(&mat_handle.0) {
            let alpha = base_alpha + (t * 0.3 + cloud.origin.x * 0.02).sin() * 0.05;
            mat.base_color = Color::srgba(1.0, 1.0, 1.0, alpha.clamp(0.05, 0.45));
        }
    }
}
...
fn setup(
    ...
    for (i, (x, y, z, sx, sz)) in [
        ...
        commands.spawn((
            ...
            CloudEntity {
                drift_speed: Vec2::new(1.5 + (i as f32 * 0.3), 0.4 + (i as f32 * 0.15)),
                origin: Vec2::new(*x, *z),
                drift_range: 150.0,
            },
        ));
    ...
    particles::spawn_campfire_emitter(
        &mut commands,
        Vec3::new(5.0, campfire_y + 2.0, 15.0),
        &gfx_settings,
    );

    // ── Ambient dust motes ──

    particles::spawn_dust_motes(&mut commands, &mut meshes, &mut materials, &gfx_settings);
...

    {
        ...
        foliage::spawn_grass(&mut commands, &mut meshes, &mut materials, &gfx_settings, &td);
    }
